...
export const newGame = () => ({
  ...
  delayedConsequences: {},
  log: ['Campaign launched.'],
});

const promptFor = (context) => ({
  Debate: 'Debate night: choose a stance under pressure.',
  'Social Media': 'Social strategy meeting: what message do you amplify this week?',
  ...
  Crisis: 'A breaking crisis dominates media coverage.',
  'Internal Conflict': 'Campaign infighting erupts between advisors and donors.',
}[context]);
...
export const generateWeeklyEvent = (state) => {
  ...
  const pool = Array.from({ length: 50 }, (_, idx) => {
    ...
    return {
      ...
      description: `Frame a ${tone} stance on ${issue}. Gains one bloc while risking another.`,
      unethical,
      effect: {
        supportShift,
